...
import { browseJobpost } from "../../utils/api/api";
...
export default function userProfile() {
    ...
    useEffect(() => {
      const fetchJobData = async () => {
        try {
          const response = await clientAxiosInstance.get(`${browseuserProfile}?id=${id}`);
    ...
    return (
      <>
            ...
            {/* <div className="w-1/3 p-20">
              <div className="pb-10">
                <div className="flex justify-center">
                  <button className="w-80 mr-10 my-3 border-1 border-teal-500 px-16 py-2 items-center rounded-xl font-semibold bg-teal-600 text-white hover:bg-teal-500" >
                    Apply now
                  </button>
                </div>
                <div className="flex justify-center">
        ...
        <button className="flex justify-center items-center bg-blue-500 text-white py-2 px-4 rounded" onClick={()=>{navigate(-1)}}>
          Cancel
        </button>
      </div>
